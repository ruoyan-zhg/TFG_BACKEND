...
class controladorItinerarios extends Controller
{
    public function formularioItinerarios(Request $request)
{
    // reponder con holamundo
    //return "Hola Mundo";
    // Crear un objeto stdClass para almacenar los datos del formulario
    $formulario = new stdClass();
    $formulario->destino = $request->destino;
    $formulario->personas = $request->personas;
    $formulario->ninos = $request->ninos;
    $formulario->dias = $request->dias;
    ...
    // Configurar cURL
    curl_setopt($ch, CURLOPT_URL, "http://localhost:5000/formularioItinerarios");  // Asegúrate de reemplazar esto con tu URL de Flask
